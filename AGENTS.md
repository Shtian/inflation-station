...
- Monthly review overview convention: keep deterministic month aggregation logic in `src/lib/monthly-review/overview.ts`, count spend from negative transactions only, include review-only months, and compute month-over-month spend deltas only when the immediate prior calendar month exists.
...
- Monthly review route convention: keep `/monthly-review` as `page.tsx` + `monthly-review-route-root.tsx`, keep timeline fetch/render state in `monthly-review-manager.tsx`, and show explicit per-month empty-state text when `reviewState` is `NOT_GENERATED`.
...
- Route composition convention: keep each route `page.tsx` as a thin entry that renders a single route-local `*-route-root.tsx` component, and keep route-root client components under 160 lines (enforced by `pnpm lint` via `scripts/check-route-client-size.mjs`).
