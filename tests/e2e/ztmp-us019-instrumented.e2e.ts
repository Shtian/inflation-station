...
async function probe(page: import("@playwright/test").Page, tag: string) {
  const state = await page.evaluate(() => ({
    ...
    providerName: (document.getElementById("new-provider-name") as HTMLInputElement | null)?.value ?? null,
    mappingVersion: (document.getElementById("new-mapping-version") as HTMLInputElement | null)?.value ?? null,
    required: [...document.querySelectorAll("input[aria-label^=\"Required source field\"]")].map((i) => [i.getAttribute("aria-label"), (i as HTMLInputElement).value]),
    alerts: [...document.querySelectorAll("[role=alert]")].map((a) => a.textContent?.trim()).filter(Boolean),
  }));
  console.log("PROBE " + tag + " " + JSON.stringify(state));
}
...
test("manages provider mappings from admin UI with validation feedback", async ({
  ...
  await page
    .getByRole("textbox", { name: "Required source field merchant signal" })
    .fill("Tekst");
  await probe(page, "before-second-create");
  await page.getByRole("button", { name: "Create provider mapping" }).click();

  try {
    await expect.poll(() => createServerActionRequestCount, { timeout: 5000 }).toBe(2);
  } finally {
    await probe(page, "after-second-create");
  }
  await expect(
